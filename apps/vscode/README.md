...
```sh
pnpm install
pnpm --filter @deepseek-ai/dsh-vscode run build   # tsc -> dist/extension.js
```
...
This produces `deepseek-harness-<version>.vsix`; install it with the VS Code `Extensions: Install from VSIX...` command.
