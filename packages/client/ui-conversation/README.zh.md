...
ChatView 还在消息流旁边声明了会话作用域的单实例 `conversation.chat.navigator` slot。聊天入口向该 slot 提供与「加载更早」按钮相同的锚定历史加载器，因此导航条可以在不移动当前阅读行的前提下加载足够多的更早历史来决定是否显示。本包只拥有空位；由纯浏览器端的 turn-navigator 插件填充。
